	...
	case "$board" in
		iptime,ax3000sm | *ax3000sm*)
			...
			if ss_identity_valid_mac "$mac"; then
				...
				SS_IDENTITY_SOURCE="mtd:Factory:0x4:mac"
		...
		glinet,gl-mt300n-v2 | gl.inet,gl-mt300n-v2 | *gl-mt300n-v2*)
			...
			for offset in 0x4 0x28 0x2e; do
				...
				if ss_identity_valid_mac "$mac"; then
					SS_IDENTITY_PROVIDER="factory_mac"
					SS_IDENTITY_SOURCE="mtd:factory:${offset}:mac"
					SS_IDENTITY_STRENGTH="hardware_soft"
					printf '%s' "$mac"
					return 0
				fi
...
ss_identity_compute_physical() {
	...
	case "$profile" in
		smartsafehub*)
			value="$(ss_identity_try_factory_device_id 2>/dev/null || true)"
			if [ -n "$value" ]; then
	...
	value="$(ss_identity_try_mtd_factory_mac "$board" 2>/dev/null || true)"
	if [ -n "$value" ]; then
		SS_IDENTITY_VALUE_SHA256="$(ss_identity_sha256 "$value")"
...
ss_identity_load() {
	[ -s "$SS_IDENTITY_FILE" ] || return 1

	# identity.env is created by SafeShield with mode 0600. It contains only
	# fixed variable names generated by this script.
	# shellcheck disable=SC1090
	. "$SS_IDENTITY_FILE"

	SS_PHYSICAL_FINGERPRINT="${PHYSICAL_FINGERPRINT:-}"
	SS_FINGERPRINT_VERSION="${FINGERPRINT_VERSION:-1}"
